...
def main(args):
    ...
    def train_path(lang):
        # args.trainpref = "dataset/wmt14_en_de/train,dataset/iwslt14.tokenized.en-de/train"
    ...
    def split_ted(lang):
        ...
        domain_nums = 5
        lines_per_file = ted_total_lines / domain_nums
        smallfile = None
        domain_file_list = []
        with open(trainpref) as bigfile:
            for lineno, line in enumerate(bigfile):
                if lineno % lines_per_file == 0:
                    if smallfile:
                        smallfile.close()
                    domain_index = lineno // lines_per_file
                    small_filename = trainpref + '.domain{}'.format(domain_index)
    ...
    def make_all(lang):
        if args.trainpref:
            train_file_list = split_ted(lang)
            for k, trainpref in enumerate(train_file_list, 1):
                outprefix = "train_domain{}".format(k)
...
def cli_main():
    ...
    parser.add_argument("--domain_nums", default=2)
